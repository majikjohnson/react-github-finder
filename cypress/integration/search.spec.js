/// <reference types="Cypress" />

describe('Github Finder search functionality', () => {
    describe('Spinner Component', () => {
        const githubSearchUsersAPI = 'https://api.github.com/search/users';

        it('should display spinner while waiting for Github API call to return', () => {
          cy.server();
          cy.route({
            method: 'GET',
            url: githubSearchUsersAPI,
            response: [{
              login: 'mojombo',
              id: 1,
              avatar_url: 'https://avatars0.githubusercontent.com/u/1?v=4',
              url: 'https://api.github.com/users/mojombo',
              html_url: 'https://github.com/mojombo',
            }],
            delay: 3000
          });
          cy.visit('http://localhost:3000');
          cy.get('input[data-test-id="searchBox"]').type('majik');
          cy.get('.btn').click();
          cy.get('div.container img').should('exist').and('have.attr', 'alt', 'Loading...');
        });
    });

    describe('Search results', () => {
        before(() => {
            cy.visit('http://localhost:3000');
            cy.get('input[data-test-id="searchBox"]').type('majik');
            cy.get('.btn').click();
        });
    
        it('should contain 30 user items', () => {
            cy.get('div[data-test-selector="userCard"]')
                .should('have.length', 30);
        });
    
        it('should have user items that contain the user avatar image', () => {
            cy.get('div[data-test-selector="userCard"]:first-child img')
                .should('have.attr', 'src', 'https://avatars0.githubusercontent.com/u/17090?v=4' );
        });
    
        it('should have user items that contain the github login', () => {
            cy.get('div[data-test-selector="userCard"]:nth-child(2) h3')
                .should('have.text', 'majikarp' );
        });
    
        it('should have user items that contain a button pointing to the user github profile page', () => {
            cy.get('div[data-test-selector="userCard"]:nth-child(3) a')
                .should('have.attr', 'href', 'https://github.com/majikang')
                .and('have.text', 'more');
        });
    });
});