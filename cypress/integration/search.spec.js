...
describe("Github Finder search functionality", () => {
    ...
    it("should display spinner while waiting for Github API call to return", () => {
      ...
      cy.get('input[data-test-id="searchBox"]').type("majik");
      cy.get(".btn").click();
  ...
  describe("Search results", () => {
    ...
    it("should contain 30 user items", () => {
      cy.get('div[data-test-selector="userCard"]').should("have.length", 30);
    });

    it("should have user items that contain the user avatar image", () => {
      cy.get('div[data-test-selector="userCard"]:first-child img').should(
        "have.attr",
        "src",
    ...
    it("should have user items that contain the github login", () => {
      cy.get('div[data-test-selector="userCard"]:nth-child(2) h3').should(
        "have.text",
        "majikarp"
      );
    });
  ...
  describe("Clear button", () => {
    ...
    it.only("should appear if there are user search results displayed", () => {
      cy.visit("http://localhost:3000");
      cy.get('input[data-test-id="searchBox"]').type("majik");
      cy.get(".btn").click();
      cy.get('button[data-test-id="clearButton"]')
        .should('exist')
        .and('have.text', 'Clear');
    });
    ...
    it("should not appear if there are no user search results displayed", () => {
        cy.visit("http://localhost:3000");
        cy.get('button[data-test-id="clearButton"]').should('not.exist');
    });
    ...
    it("should clear the search results", () => {});
        ...
        cy.get(".btn").click();
        cy.get('div[data-test-selector="userCard"]').should("have.length", 30);
        cy.get('button[data-test-id="clearButton"]').click();
